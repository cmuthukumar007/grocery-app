...
<template>
 ...
 const groceryIndex = tableData.findIndex((item: any) => String(item.id) === String(productid))
 ...
 onMounted(() => {
    const productId = useRouter().currentRoute.value.params.id;
    const tableData = getTableData();
    const filterResult = tableData.filter( (item: any) => String(item.id) === String(productId) )
     if(filterResult && filterResult.length > 0) { 
        const [product] = filterResult;
         Object.assign(oldData.value, product)
